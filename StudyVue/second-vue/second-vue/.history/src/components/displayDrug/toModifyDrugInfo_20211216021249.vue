...
export default {
    ...
    methods: {
        modify() {
            ...
            for(let i = 0; i < that.checkList[i]; ++i) {
                mySelect.set(that.checkList[i], true);
                if(oldSelect.has(that.checkList[i])) {
            ...
            for(let key of oldSelect.keys()) {
                if(!oldSelect.get(key)) {
                    newSelect.set(key, falue]);
                }
            }

            //记录以前没有的, 但现在增加的, 增加的value为true
            for(let i = 0; i < that.response.data.length; ++i) {
                if(mySelect.has(that.response.data[i])) {
                    mySelect.set(that.response.data[i], false);
                }
            }
