                ...
                .then(function(response) {
                    ...
                    for(let i = 0 ; i < that.tableData.length; ++i) {
                        let param1 = qs.stringify({
                            doctorId : that.tableData[i].doctorId
                        });
                        that.$axios.post('http://localhost:8080/patient/getRegistrationNumber', param1)
                            .then(function(response){
                                that.tableData[i].registrationNumber = response.data;
                            })
                    }
                });
